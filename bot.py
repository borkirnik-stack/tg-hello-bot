...
async def notion_query_database(args: dict) -> str:
    ...
    if is_projects:
        CLOSED_STATUSES = {
            "Закрытие", "Закрыт но не оплачен", "Закрыт прод-р неоплачен",
            "Заморожено", "Закрыто 2023", "Закрыто 2024", "Закрыто 2025", "Отменено",
        }
        by_status = {}
        for r in results:
            props = r.get("properties", {})
        ...
        # Порядок статусов как на канбане в Notion
        status_order = [
            "Лиды / брифинг", "Дебрифинг",
            "Взято в работу / Концепция / Поиск интеграции",
            "Переторжка", "Ждем результаты",
            "Запуск проекта", "Аккр-и / Рамочные",
            "Производство", "Постпродакшн съемочные",
        ]
        lines = []
        shown_statuses = set()
